CREATE OR REPLACE FUNCTION populate_createdby2() RETURNS VOID AS $$
    ...
    BEGIN               
	 ...
	 -- Step 1: Create a temp table 'auditlogrecord_temp'
     CREATE TABLE auditlogrecord_temp
        (
        id bigint NOT NULL,
    ...
    CREATE INDEX auditlog_temp_index ON auditlogrecord_temp USING btree (entityid);

    -- Step 2: Load data into 'auditlogrecord_temp' from 'auditlogrecord' table
    insert into auditlogrecord_temp
    select * from auditlogrecord where type ='INSERT' 
    ...
    if exists(select * from information_schema.tables where table_name = 'auditlogrecord_022713') then
        insert into auditlogrecord_temp
        select * from auditlogrecord_022713 where type ='INSERT' 
        and entityname in ('Organization','HealthCareFacility','ResearchOrganization','OversightCommittee','IdentifiedOrganization','Person','ClinicalResearchStaff','HealthCareProvider','OrganizationalContact','IdentifiedPerson');
    end if;
    ...
    -- Step3: update the records to have 'createdBy' set
        ...
        FOR org_id IN SELECT id FROM organization WHERE created_by_id is null LOOP              
            SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = org_id );
        ...
        FOR hcf_id IN SELECT id FROM healthcarefacility WHERE created_by_id is null LOOP                
            SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = hcf_id );
        ...
        FOR ro_id IN SELECT id FROM researchorganization WHERE created_by_id is null LOOP               
           SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = ro_id );
        ...
        FOR over_comm_id IN SELECT id FROM oversightcommittee WHERE created_by_id is null LOOP              
           SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = over_comm_id );
        ...
        FOR io_id IN SELECT id FROM identifiedorganization WHERE created_by_id is null LOOP             
           SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = io_id );
        ...
        FOR per_id IN SELECT id FROM person WHERE created_by_id is null LOOP                
           SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = per_id);
        ...
        FOR crs_id IN SELECT id FROM clinicalresearchstaff WHERE created_by_id is null LOOP             
           SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = crs_id );
        ...
        FOR hcp_id IN SELECT id FROM healthcareprovider WHERE created_by_id is null LOOP                
           SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = hcp_id);
        ...
        FOR org_cont_id IN SELECT id FROM organizationalcontact WHERE created_by_id is null LOOP                
           SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = org_cont_id);
        ...
        FOR ip_id IN SELECT id FROM identifiedperson WHERE created_by_id is null LOOP               
           SELECT INTO created_by_user_id user_id FROM CSM_USER WHERE LOWER(LOGIN_NAME) = (SELECT LOWER(username) FROM auditlogrecord_temp WHERE entityid = ip_id);
           UPDATE identifiedperson SET created_by_id = created_by_user_id WHERE id = ip_id;    
        END LOOP;

        
    -- Step4: Drop the temp table
        drop table auditlogrecord_temp;
