package gov.nih.nci.po.webservices.security;
 ...
 *
 * Credentials are parsed using {@link UsernamePasswordProvider} instances.  This class
 * will iterate through the hardwired set of providers (currently there are two - one
 * for typical Basic auth via HTTP headers, and one to mimic WS-Security UsernameToken auth).
 *
...
public class UsernameTokenGridAuthenticator extends AuthenticatorBase {

    private static final Logger LOG = Logger.getLogger(UsernameTokenGridAuthenticator.class);
     ...
     * {@inheritDoc}
     *
     * This implementation will iterate through the registered credential providers.
     * The first provider that returns a non-null value is utilized.
     */
    @Override
    protected boolean authenticate(Request request, HttpServletResponse response, LoginConfig loginConfig)
            throws IOException {

        if (usernamePasswordProviderList == null) {
            usernamePasswordProviderList = new ArrayList<UsernamePasswordProvider>();
            usernamePasswordProviderList.add(new BasicUsernamePasswordProvider());
            usernamePasswordProviderList.add(new WsSecurityUsernamePasswordProvider());
        }
     ...
     * @throws IOException
     */
    private void sendChallenge(Request request, HttpServletResponse response, LoginConfig loginConfig)
            throws IOException {
        MessageBytes authenticate =
                request.getResponse().getCoyoteResponse().getMimeHeaders()
                        .addValue(
                                BasicAuthenticator.AUTHENTICATE_BYTES,
                                0,
     ...
     *
     * The JAAS login chain is invoked after Dorian, and the resulting principal is returned.
     *
     * @param basicAuthInfo The credentials to use.
    ...
    private Principal doAuthentication(BasicAuthInfo basicAuthInfo) {
        //do grid auth
        Principal principal
                = DorianService.getInstance().authenticate(
                    basicAuthInfo.getUsername(),
                    basicAuthInfo.getPassword(),
                    context.getRealm()
                );
        ...
        if (principal != null) {
          principal = context.getRealm().authenticate(principal.getName(), basicAuthInfo.getPassword());
        }
