...
<s:if test="%{isCreate}">
    <title>Create Clinical Research Staff</title>
</s:if>
<s:else>
   <c:if test="${fn:length(role.changeRequests) > 0}">
      <title>Clinical Research Staff Details - Comparison</title>
   </c:if>
   <c:if test="${fn:length(role.changeRequests) == 0}">
      <title>Clinical Research Staff Details</title>
   </c:if>
</s:else>
    ...
    <h2>Clinical Research Staff Information</h2>
		...
		<div class="boxouter">
			<s:if test="%{isCreate}">
				...
				<h2>Clinical Research Staff Role Information</h2>
			</s:if> <s:else>
				<s:set name="formAction"
