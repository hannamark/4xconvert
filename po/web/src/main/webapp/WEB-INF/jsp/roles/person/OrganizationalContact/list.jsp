...
<c:url value="/protected/ajax/roles/person/OrganizationalContact/results/list.action" var="sortUrl" />
<ajax:displayTag id="roleSearchResults" ajaxFlag="true" tableClass="data">
    <display:table class="data" sort="list" pagesize="${initParam['defaultPageSize']}" uid="row" name="results" requestURI="${sortUrl}" >
        ...
        <display:column titleKey="th.action" class="action">
            <c:url var="editUrl" value="/protected/roles/person/OrganizationalContact/input.action">
                <c:param name="person" value="${person.id}"/>
                <c:param name="role.id" value="${row.id}"/>
            </c:url>
            <po:buttonRow>
                <po:button href="${editUrl}" style="edit" text="Edit" id="edit_organizationalContact_id_${row.id}"/>
            </po:buttonRow>
        </display:column>
    </display:table>
</ajax:displayTag>
