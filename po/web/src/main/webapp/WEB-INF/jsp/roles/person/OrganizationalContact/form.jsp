...
<s:else>
   <c:if test="${fn:length(role.changeRequests) > 0}">
      <title><s:text name="organizationalContact"/> Details - Comparison</title>
   </c:if>
   <c:if test="${fn:length(role.changeRequests) == 0}">
      <title><s:text name="organizationalContact"/> Details</title>
   </c:if>
</s:else>
		...
		<div class="boxouter">
			<s:if test="%{isCreate}">
				<s:set name="formAction"
					value="'roles/person/OrganizationalContact/add.action'" />
				<h2><s:text name="organizationalContact"/> Role Information</h2>
			</s:if> <s:else>
				<s:set name="formAction"
					value="'roles/person/OrganizationalContact/edit.action'" />
				<h2><s:text name="organizationalContact"/> Role Information</h2>
			</s:else>
		    <div class="box_white">
				<s:actionerror/>
...
<div style="clear:left;"></div>    
    ...
    <po:buttonRow>
       ...
       <c:url var="managePage" value="/protected/roles/person/OrganizationalContact/start.action">
