...
<c:if test="${sessionScope.indIdeAddList != null}">
    <c:if test="${fn:length(sessionScope.indIdeAddList) > 0}">
	<display:table class="data" sort="list"  uid="row"  name="${sessionScope.indIdeAddList}" requestURI="submitTrialdisplayIndIde.action">
		<display:column title="IndIde Type" property="indIde"  headerClass="sortable"/>
		<display:column escapeXml="true" title="Number" property="number"  headerClass="sortable"/>
