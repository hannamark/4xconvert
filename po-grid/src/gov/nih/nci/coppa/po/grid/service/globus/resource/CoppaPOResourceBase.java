package gov.nih.nci.coppa.po.grid.service.globus.resource;
...
import gov.nih.nci.coppa.po.grid.common.CoppaPOConstants;
import gov.nih.nci.coppa.po.grid.stubs.CoppaPOResourceProperties;
import gov.nih.nci.coppa.po.grid.service.CoppaPOConfiguration;
import java.io.File;
 ...
 * 
 */
public abstract class CoppaPOResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(CoppaPOResourceBase.class);

	private CoppaPOResourceConfiguration configuration;
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((CoppaPOResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(CoppaPOConstants.SERVICEMETADATA);
	...
	public CoppaPOResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (CoppaPOResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((CoppaPOResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
