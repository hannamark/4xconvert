...
/**
 * @author smatyas THIS CLASS WAS TAKEN DIRECTLY FROM caGrid v1.2 -- add modified to allow for easy BaseFaultType
 *         population, see toFault method
 * 
 *         This class provides convenience functions around BaseFault API. It also provides a common way of including
 *         stack traces with Faults. A stack trace of a Fault is added as a chained BaseFault with an error code dialect
 *         attribute set to {@link #STACK_TRACE STACK_TRACE}. A regular Java exception is automatically converted into a
 *         BaseFault with the description of exception message and with a chained BaseFault with {@link #STACK_TRACE
 *         STACK_TRACE} error code dialect.
 */
public final class POFaultHelper {
     ...
     * @param fault fault
     */
    public POFaultHelper(BaseFaultType fault) {
        this(fault, true);
    }
     ...
     *            STACK_TRACE}.
     */
    public POFaultHelper(BaseFaultType fault, boolean convertStackTrace) {
        if (fault == null) {
     ...
     * @param exception the actual expection to convert to a type <E>
    ...
    public static <E extends BaseFaultType> E toFault(E fault, Throwable exception) {
        POFaultHelper helper = new POFaultHelper(fault, false);
