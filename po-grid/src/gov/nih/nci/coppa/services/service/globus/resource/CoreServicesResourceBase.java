 ...
 * 
 */
public abstract class CoreServicesResourceBase extends ReflectionResource implements Resource
                                                  {

	static final Log logger = LogFactory.getLog(CoreServicesResourceBase.class);

	private CoreServicesResourceConfiguration configuration;
    ...
    public CoreServicesResourceBase() {
    }
	...
	public gov.nih.nci.cagrid.metadata.ServiceMetadata getServiceMetadata(){
		return ((CoreServicesResourceProperties) getResourceBean()).getServiceMetadata();
	}
	
	public void setServiceMetadata(gov.nih.nci.cagrid.metadata.ServiceMetadata serviceMetadata ) throws ResourceException {
        ResourceProperty prop = getResourcePropertySet().get(CoreServicesConstants.SERVICEMETADATA);
	...
	public CoreServicesResourceConfiguration getConfiguration() {
		...
		try {
			...
			this.configuration = (CoreServicesResourceConfiguration) initialContext.lookup(jndiName);
		} catch (Exception e) {
	...
	private void loadServiceMetadataFromFile() {
      if(getServiceMetadata()==null){
		try {
			...
			((CoreServicesResourceProperties) this.getResourceBean()).setServiceMetadata((gov.nih.nci.cagrid.metadata.ServiceMetadata) Utils.deserializeDocument(dataFile.getAbsolutePath(),
