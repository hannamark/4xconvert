package gov.nih.nci.coppa.services.outcomes.user.service.globus.resource;

import gov.nih.nci.coppa.services.outcomes.user.common.UserConstants;
import gov.nih.nci.coppa.services.outcomes.user.stubs.UserResourceProperties;
 ...
 * 
 */
public class UserResourceHome extends SingletonResourceHomeImpl implements Initializable {

	static final Log logger = LogFactory.getLog(UserResourceHome.class);
    private static final UUIDGen UUIDGEN = UUIDGenFactory.getUUIDGen();

	public Resource createSingleton() {
		logger.info("Creating a single resource.");
		try {
		    UserResourceProperties props = new UserResourceProperties();
			...
			if (resource instanceof PersistenceCallback) {
                  ...
                  try{
                    ((PersistenceCallback) resource).load(null);
			          ...
			          resource.initialize(props, UserConstants.RESOURCE_PROPERTY_SET, UUIDGEN.nextUUID());
			      }
            } else {
                    resource.initialize(props, UserConstants.RESOURCE_PROPERTY_SET, UUIDGEN.nextUUID());
            }
	...
	public Resource find(ResourceKey key) throws ResourceException {
		UserResource resource = (UserResource) super.find(key);
		return resource;
	}
     ...
     */
    public UserResource getAddressedResource() throws Exception {
        UserResource thisResource;
        thisResource = (UserResource) ResourceContext.getResourceContext().getResource();
        return thisResource;
    }
}