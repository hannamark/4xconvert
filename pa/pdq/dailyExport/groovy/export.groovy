...
def sourceConnection = Sql.newInstance(props['pa.jdbc.url'], props['datawarehouse.pa.source.db.username'], 
    props['datawarehouse.pa.source.db.password'], props['datawarehouse.pa.source.jdbc.driver'])
...
sourceConnection.eachRow(collabTrialsSQL) { spRow ->
	...
	def out = new FileOutputStream("out/" + spRow.nciId +  ".xml")
        ...
        xml.nci_specific_information {
            ...
            xml.summary_4_funding_sponsor_source {
                if (spRow.sum4OrgId != null) {
                    ...
                    addressAndPhoneDetail(xml, sum4Org)
                }
            }
        }
		...
		sourceConnection.eachRow(Queries.partSitesSQL, [studyProtocolID]) { row ->
			xml.location {
				xml.facility {
                    ...
                    addressAndPhoneDetail(xml, hcfRow)
				}
...
void addressAndPhoneDetail(MarkupBuilder xml, Object row) {  
    xml.address {
