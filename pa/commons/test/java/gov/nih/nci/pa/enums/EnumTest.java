...
public class EnumTest {
    ...
    public void test()
    {
        ...
        testMethods(AssayTypeCode.class);
        testMethods(AssayUseCode.class);
        testMethods(AssayPurposeCode.class);
        testMethods(TissueSpecimenTypeCode.class);
        testMethods(TissueCollectionMethodCode.class);
    }
    ...
    public <C, T extends CodedEnum<C>> void testMethods(Class<T> clazz){
        ...
        if (clazz.getName().equals("gov.nih.nci.pa.enums.AssayTypeCode")) {
            String[] names = AssayTypeCode.getDisplayNames();
            assertEquals(AssayTypeCode.values().length, names.length);
            assertEquals("PCR", AssayTypeCode.PCR.getName());
            assertEquals("PCR", AssayTypeCode.PCR.getCode());
        }
        if (clazz.getName().equals("gov.nih.nci.pa.enums.AssayUseCode")) {
            String[] names = AssayUseCode.getDisplayNames();
            assertEquals(AssayUseCode.values().length, names.length);
            assertEquals("RESEARCH", AssayUseCode.RESEARCH.getName());
            assertEquals("Research", AssayUseCode.RESEARCH.getCode());

        }
        if (clazz.getName().equals("gov.nih.nci.pa.enums.AssayPurposeCode")) {
            String[] names = AssayPurposeCode.getDisplayNames();
            assertEquals(AssayPurposeCode.values().length, names.length);
            assertEquals("TREATMENT_ASSIGNMENT", AssayPurposeCode.TREATMENT_ASSIGNMENT.getName());
            assertEquals("Treatment Assignment", AssayPurposeCode.TREATMENT_ASSIGNMENT.getCode());
        }
        if (clazz.getName().equals("gov.nih.nci.pa.enums.TissueSpecimenTypeCode")) {
            String[] names = TissueSpecimenTypeCode.getDisplayNames();
            assertEquals(TissueSpecimenTypeCode.values().length, names.length);
            assertEquals("PLASMA", TissueSpecimenTypeCode.PLASMA.getName());
            assertEquals("Plasma", TissueSpecimenTypeCode.PLASMA.getCode());
        }
        if (clazz.getName().equals("gov.nih.nci.pa.enums.TissueCollectionMethodCode")) {
            String[] names = TissueCollectionMethodCode.getDisplayNames();
            assertEquals(TissueCollectionMethodCode.values().length, names.length);
            assertEquals("MANDATORY", TissueCollectionMethodCode.MANDATORY.getName());
            assertEquals("Mandatory", TissueCollectionMethodCode.MANDATORY.getCode());
        }
