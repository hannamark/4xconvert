...
<display:table class="data" decorator="gov.nih.nci.pa.decorator.PADisplayTagDecorator" sort="list" pagesize="10" id="row"
    name="records" requestURI="inboxProcessingquery.action" export="false">
    ...
    <display:column escapeXml="true" titleKey="studyProtocol.milestoneDate" property="studyMilestoneDate" format="{0,date,MM/dd/yyyy}" sortable="true" headerClass="sortable"/>
    ...
    <display:column escapeXml="true" titleKey="studyProtocol.documentWorkflowStatusDate" property="documentWorkflowStatusDate" format="{0,date,MM/dd/yyyy}" sortable="true" headerClass="sortable"/>
    ...
    <display:column escapeXml="true" titleKey="inboxTrial.updatedDate" property="updatedDate" format="{0,date,MM/dd/yyyy}" sortable="true" headerClass="sortable"/>
    <display:column escapeXml="true" titleKey="inboxTrial.updateReason" property="updatedComments"  headerClass="sortable"/>
    <c:if test="${(sessionScope.role == 'Abstractor') || (sessionScope.role == 'SuAbstractor')}">
        <display:column escapeXml="true" class="title" titleKey="studyProtocol.action" href="studyProtocolview.action" property="action"  paramId="studyProtocolId"
            paramProperty="studyProtocolId" sortable="true" headerClass="sortable"/>
    </c:if>
