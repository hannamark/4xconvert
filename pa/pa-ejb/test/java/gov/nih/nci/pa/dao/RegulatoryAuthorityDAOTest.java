...
public class RegulatoryAuthorityDAOTest {
	...
	RegulatoryInformationDAO informationDAO; 
	Country country;
    ...
    public void getDistinctCountry() throws PAException {
    	...
    	assertEquals(8, new Long(testList.size()).longValue());    	
    }
    ...
    public void getRegulatoryInformationForEditTest() throws PAException{
        ...
        Long id = sp.getId();
    	RegulatoryAuthority authority = new RegulatoryAuthority();
    	authority.setAuthorityName("AIMMS_IND_456XSD1QA34");
    	authority.setCountry(country);
    	TestSchema.addUpdObject(authority);        
        StudyRegulatoryAuthority studyAuthority = new StudyRegulatoryAuthority();
        studyAuthority.setRegulatoryAuthorityID(authority.getId());
    	...
    	assertEquals(orgDTO.getName(), authority.getAuthorityName());
    }
}
